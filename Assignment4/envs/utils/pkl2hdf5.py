...
def _camera_video_path(video_path, camera_name):
    video_dir = os.path.dirname(video_path)
    video_name = os.path.basename(video_path)
    return os.path.join(video_dir, camera_name, video_name)
...
def pkl_files_to_hdf5_and_video(pkl_files, hdf5_path, video_path):
    ...
    for camera_name in ("head_camera", "left_camera", "right_camera"):
        ...
        if "rgb" not in camera_data:
            continue
        out_path = video_path if camera_name == "head_camera" else _camera_video_path(video_path, camera_name)
        images = np.array(camera_data["rgb"])
        images_to_video(images, out_path=out_path)
        if camera_name == "head_camera":
            images_to_video(images, out_path=_camera_video_path(video_path, camera_name))
