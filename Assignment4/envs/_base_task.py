...
class Base_Task(gym.Env):
    ...
    def _init_task_env_(self, table_xy_bias=[0, 0], table_height_bias=0, **kwags):
        ...
        embodiment_config_overrides = kwags.get("embodiment_config_overrides", {})
        if embodiment_config_overrides:
            for config_key in ("left_embodiment_config", "right_embodiment_config"):
                if config_key in kwags:
                    config = deepcopy(kwags[config_key])
                    config.update(deepcopy(embodiment_config_overrides))
                    kwags[config_key] = config
        left_embodiment_config = kwags.get("left_embodiment_config", {})
        self.arm_control_substeps = max(
            1,
            int(kwags.get("arm_control_substeps", left_embodiment_config.get("arm_control_substeps", 1))),
        )
        self.world_pcd = None
        ...
        self.create_table_and_wall(
            ...
            table_height=kwags.get("table_height", 0.74),
        ...
        self.cameras.set_camera_hidden_link_entities([self.robot.left_entity, self.robot.right_entity])
        ...
        if self.eval_mode:
            with open(os.path.join(CONFIGS_PATH, "_eval_step_limit.yml"), "r") as f:
                try:
                    ...
                    self.step_lim = 1000
            if kwags.get("eval_step_lim") is not None:
                self.step_lim = int(kwags["eval_step_lim"])
    ...
    def check_stable(self):
        ...
        if hasattr(self, "robot"):
            ...
            for entity in [self.robot.left_entity, self.robot.right_entity]:
                ...
                robot_entity_states.append((entity, entity.get_qpos().copy()))

        def step_scene_with_robot_qf():
            if hasattr(self, "robot"):
        ...
        check(500)
        for entity, qpos in robot_entity_states:
    ...
    def create_table_and_wall(self, table_xy_bias=[0, 0], table_height=0.74, table_length=1.2, table_width=0.7):
    ...
    def get_cluttered_table(self, cluttered_numbers=10, xlim=None, ylim=None, zlim=[0.741]):
        self.record_cluttered_objects = []  # record cluttered objects
        if xlim is None:
            x_margin = min(0.01, max(0.0, self.table_length / 2 - 0.01))
            xlim = [-self.table_length / 2 + x_margin, self.table_length / 2 - x_margin]
        if ylim is None:
            y_margin = min(0.01, max(0.0, self.table_width / 2 - 0.01))
            ylim = [-self.table_width / 2 + y_margin, self.table_width / 2 - y_margin]
    ...
    def _update_render(self):
        ...
        left_parent_name, right_parent_name = self.cameras.get_wrist_camera_parent_link_names()
        left_camera_parent = (
            self.robot.left_entity.find_link_by_name(left_parent_name) if left_parent_name else self.robot.left_camera
        )
        right_camera_parent = (
            self.robot.right_entity.find_link_by_name(right_parent_name) if right_parent_name else self.robot.right_camera
        )
        if left_camera_parent is None:
            left_camera_parent = self.robot.left_camera
        if right_camera_parent is None:
            right_camera_parent = self.robot.right_camera
        self.cameras.update_wrist_camera(left_camera_parent.get_pose(), right_camera_parent.get_pose())
        self.scene.update_render()
    ...
    def left_move_to_pose(
        ...
        if self.need_plan:
            left_result = self.robot.left_plan_path(pose, constraint_pose=constraint_pose, last_qpos=last_qpos)
    ...
    def right_move_to_pose(
        ...
        if self.need_plan:
            right_result = self.robot.right_plan_path(pose, constraint_pose=constraint_pose, last_qpos=last_qpos)
    ...
    def _qpos_with_arm_plan_endpoint(self, arm_tag, result):
                ...
                if left is not None:
                    if left.action == "move":
                        control_seq["left_arm"] = self.left_move_to_pose(
                            ...
                            last_qpos=last_left_qpos,
                        )
                        if control_seq["left_arm"] is not None and self.plan_success is not False:
                            last_left_qpos = self._qpos_with_arm_plan_endpoint("left", control_seq["left_arm"])
                ...
                if right is not None:
                    if right.action == "move":
                        control_seq["right_arm"] = self.right_move_to_pose(
                            ...
                            last_qpos=last_right_qpos,
                        )
                        if control_seq["right_arm"] is not None and self.plan_success is not False:
                            last_right_qpos = self._qpos_with_arm_plan_endpoint("right", control_seq["right_arm"])
    ...
    def take_dense_action(self, control_seq, save_freq=-1):
        ...
        prev_left_arm_target = None
        prev_right_arm_target = None
        for control_idx in range(max_control_len):
            left_target_position = None
            left_target_velocity = None
            right_target_position = None
            right_target_velocity = None
            control_has_arm = False
            if (left_arm is not None and control_idx < left_arm["position"].shape[0]):  # control left arm
                left_target_position = left_arm["position"][control_idx]
                left_target_velocity = left_arm["velocity"][control_idx]
                control_has_arm = True

            if (right_arm is not None and control_idx < right_arm["position"].shape[0]):  # control right arm
                right_target_position = right_arm["position"][control_idx]
                right_target_velocity = right_arm["velocity"][control_idx]
                control_has_arm = True

            substeps = self.arm_control_substeps if control_has_arm else 1
            for substep_idx in range(substeps):
                ratio = float(substep_idx + 1) / float(substeps)
                if left_target_position is not None:
                    if prev_left_arm_target is None:
                        command_position = left_target_position
                    else:
                        command_position = prev_left_arm_target + (left_target_position - prev_left_arm_target) * ratio
                    self.robot.set_arm_joints(
                        command_position,
                        left_target_velocity / substeps,
                        "left",
                    )
                ...
                if right_target_position is not None:
                    if prev_right_arm_target is None:
                        command_position = right_target_position
                    else:
                        command_position = prev_right_arm_target + (right_target_position - prev_right_arm_target) * ratio
                    self.robot.set_arm_joints(
                        command_position,
                        right_target_velocity / substeps,
                        "right",
                    )
            ...
            if left_target_position is not None:
                prev_left_arm_target = left_target_position
            if right_target_position is not None:
                prev_right_arm_target = right_target_position
    ...
    def take_action(self, action, action_type:Literal['qpos', 'ee']='qpos'):  # action_type: qpos or ee
        ...
        while now_left_id < left_n_step or now_right_id < right_n_step:
            ...
            self.robot.hold_passive_gripper_mimic("left")
            self.robot.hold_passive_gripper_mimic("right")
            self.scene.step()
            self._update_render()
